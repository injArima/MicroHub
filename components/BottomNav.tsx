...
const BottomNav: React.FC<BottomNavProps> = ({ currentRoute, onNavigate }) => {
  ...
  return (
    <div className="fixed bottom-6 left-0 right-0 px-6 z-50 pointer-events-none">
      <div className="mx-auto max-w-[24rem] bg-[#27272a] rounded-full p-2 shadow-2xl border border-white/5 pointer-events-auto flex justify-between items-center px-4">
        
        <button 
            onClick={() => onNavigate(AppRoute.HOME)}
            className={`p-3 rounded-full transition-all duration-300 ${getBgClass(AppRoute.HOME)}`}
        >
          <Home className={`w-5 h-5 ${getIconColor(AppRoute.HOME)}`} />
        </button>
        ...
        >
          <ListTodo className={`w-5 h-5 ${getIconColor(AppRoute.TASKS)}`} />
        </button>
        ...
        >
          <BookOpen className={`w-5 h-5 ${getIconColor(AppRoute.JOURNAL)}`} />
        </button>
        
        <button 
            onClick={() => onNavigate(AppRoute.MOVIES)}
            className={`p-3 rounded-full transition-all duration-300 ${getBgClass(AppRoute.MOVIES)}`}
        >
          <Film className={`w-5 h-5 ${getIconColor(AppRoute.MOVIES)}`} />
        </button>
        ...
        >
          <MessageSquare className={`w-5 h-5 ${getIconColor(AppRoute.AI_CHAT)}`} />
        </button>
        ...
        >
          <ImageIcon className={`w-5 h-5 ${getIconColor(AppRoute.IMAGE_GEN)}`} />
        </button>
