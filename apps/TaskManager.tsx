    ...
    <div className="w-full max-w-md mx-auto min-h-screen pb-32 pt-8 px-6 flex flex-col">
      ...
      <div className="flex justify-between items-center mb-8">
        ...
        <div className="flex items-center gap-3">
             {saveStatus === 'saving' && <Loader2 size={16} className="animate-spin text-[var(--primary)]" />}
             {saveStatus === 'saved' && <CheckCircle size={16} className="text-[var(--primary)]" />}
      ...
      <div className="mb-8">
          <h1 className="text-3xl font-light text-white mb-2">My<br/><span className="font-medium text-[var(--primary)]">Schedule</span></h1>
      </div>

      {/* Date Strip */}
      <div className="flex justify-between gap-3 overflow-x-auto no-scrollbar mb-8 pb-2">
          {dates.map((d, i) => (
              <div key={i} className={`flex flex-col items-center justify-center min-w-[56px] h-[72px] rounded-[22px] cursor-pointer transition-all border ${d.active ? 'bg-[var(--primary)] border-[var(--primary)] text-black shadow-[0_0_15px_rgba(var(--primary-rgb),0.3)]' : 'glass-card border-white/5 text-gray-500 hover:bg-white/5'}`}>
        ...
        {tasks.map((task, idx) => {
            ...
            return (
                <div key={task.id} className={`
                    ...
                    ${isActive ? 'bg-[var(--primary)] text-black shadow-[0_4px_20px_rgba(var(--primary-rgb),0.2)]' : 'glass-card text-gray-300 hover:bg-white/5'}
                `}>
                    <div className={`
                    ...
                    `}>
                        {isActive ? <CheckCircle size={20} className="text-[var(--primary)]" fill="black" /> : <Circle size={20} className="text-gray-500" />}
                    </div>
